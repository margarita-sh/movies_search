...
public searchFilmByGenres(id: number): Observable<any> {
	console.log(id);
	return this._http.get(`https://api.themoviedb.org/3/discover/movie?api_key=${this._mykey}&with_genres=${id}`).pipe(
		map((item: any) => item.results));
}
}
